...
This table presents the fit between the prediction variables and the response variable ($Balance$) determined from the $cv.glmnet()$ and $pcr()$ functions, for each of our 3 models (Ridge, Lasso, PCR).

After finding the 5 predictive functions, we found the MSE's for each model:

\begin{center}
<<echo=FALSE, results='asis'>>=
MSE <- c(MSE_ridge, lasso_mse, MSE_pcr)
Model <- c('Ridge', 'Lasso', 'PCR')
